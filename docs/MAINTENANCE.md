...
The earlier `niblet-apps` import began August 12, 2026 with parentless history, so its exact imported upstream commit is not established. The public `community-apps` fork was created September 4. Its initial compatibility port touched 247 files across 187 apps, followed by further downstream work. This fork retains upstream ancestry.
