...
Make changes on a branch or personal fork and open a pull request against `main`. This applies to maintainers as well as other contributors. Do not push contributions directly to `main`. Preserve creator credits and identify the actual contributor through normal Git authorship. Describe apps as community contributions; distinguish app ownership from the services that host or run them.
